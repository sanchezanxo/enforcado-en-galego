// api.js — chamadas ao backend. Token CSRF cacheado en memoria.
...
export async function gardarPartida(payload) {
  ...
  const j = await r.json();
  if (!r.ok || !j.ok) {
    if (j.codigo && j.codigo.startsWith('CSRF')) {
      // Tentar refrescar token unha vez.
      csrfToken = null;
    }
...
// UUID v4 simple (non criptográfico) para identificar a partida no cliente.
export function uuidV4() {
  if (crypto && typeof crypto.randomUUID === 'function') return crypto.randomUUID();
  // Fallback: 16 bytes aleatorios.
